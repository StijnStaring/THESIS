...
% This is when use also the Amesim model
global Ts Tf V0
Ts = 0.01;
Tf = 40;
font = 18;
...
% while rec <= 1
    ...
    for k = 1:1:length(file_list)
        ...
        [data_planned,lambda_sol,x_sol] = optim_weights_ideal(theta,curr_data,rec,N,file);
        data_list_ID{1,k}.lam_sol = lambda_sol;
        data_list_ID{1,k}.x_sol = lambda_sol;
        V0 = data_planned.vx_cl(1,1);
        data_s = data_planned;
...
legend_names = cell(1,3*length(file_list));
for k = 1:1: length(file_list)
    file = file_list{1,k};
    name1 = "Data: " + file(6:11)+"-"+ file(13:17);
    legend_names{1,k} = name1;
end
for k = 1:1: length(file_list)
    file = file_list{1,k};
    name1 = "Init: " + file(6:11)+"-"+ file(13:17);
    legend_names{1,length(file_list)+k} = name1;
end
for k = 1:1: length(file_list)
    file = file_list{1,k};
    name1 = "Learned: " + file(6:11)+"-"+ file(13:17);
    legend_names{1,2*length(file_list)+k} = name1;
end
for k = 1:1: length(file_list)
    file = file_list{1,k};
...
set(gca,'fontsize',font_ax,'fontweight','bold')
xlabel("Time [s]", 'fontsize',font,'fontweight','bold')
ylabel("Longitudinal acceleration [m/s²]", 'fontsize',font,'fontweight','bold')
grid on
...
set(gca,'fontsize',font_ax,'fontweight','bold')
xlabel("Time [s]", 'fontsize',font,'fontweight','bold')
ylabel("Lateral acceleration [m/s²]", 'fontsize',font,'fontweight','bold')
grid on
...
set(gca,'fontsize',font_ax,'fontweight','bold')
xlabel("Time [s]", 'fontsize',font,'fontweight','bold')
ylabel("Longitudinal jerk [m/s³]", 'fontsize',font,'fontweight','bold')
grid on
...
set(gca,'fontsize',font_ax,'fontweight','bold')
xlabel("Time [s]", 'fontsize',font,'fontweight','bold')
ylabel("Lateral jerk [m/s³]", 'fontsize',font,'fontweight','bold')
grid on
