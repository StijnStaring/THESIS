...
throttle = U[0, :]
delta = U[1, :]
ref = opti.parameter(1,N+1)
opti.set_value(ref,vx_guess[0,:])
...
# diff = 0
# for i in plt.arange(0,len(vx_guess),1):
#     diff = diff + (vx[i]-vx_guess[i])**2

# diff = (vx[:] - vx_guess[:])*transpose(vx[:] - vx_guess[:])

opti.minimize(100*sumsqr(vx-ref[0,0:N+1])+sumsqr(y)+sumsqr(vy)+sumsqr(psi)+sumsqr(psi_dot))
# Set guesses
