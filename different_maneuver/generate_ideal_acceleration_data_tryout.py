...
Producing different acceleration maneuvers: different longitudinal start velocities
time_range: free time range
speed_range: [80-110] km/hour
...
from import_ideal_acceleration_data import import_ideal_acceleration_data
from define_plots import define_plots
from derivative import derivative
# from generate_delta_guess import generate_delta_guess
from casadi import *

# data_cl = import_ideal_acceleration_data()
...
time_guess = plt.arange(0,T_guess+T_guess/N,T_guess/N)
time_guess = time_guess[plt.newaxis,:]
vx_guess = a_guess*time_guess + vx_start
x_guess = a_guess*time_guess**2/2+vx_start*time_guess
...
# delta_guess = signal.resample(data_cl['delta_cl'],N).T
...
for vx_start in vx_start_a:
    for width_road in width_road_a:
        ...
        # time_debug = plt.linspace(0,opti.debug.value(T),N+1)
        # plt.figure('x')
        # plt.plot(time_debug, opti.debug.value(x))
        #
        # plt.figure('y')
        # plt.plot(time_debug, opti.debug.value(y))
        #
        # plt.figure('vx')
        # plt.plot(time_debug, opti.debug.value(vx))
        #
        # plt.figure('vy')
        # plt.plot(time_debug, opti.debug.value(vy))
        #
        # plt.figure('psi')
        # plt.plot(time_debug, opti.debug.value(psi))
        #
        # plt.figure('psi_dot')
        # plt.plot(time_debug, opti.debug.value(psi_dot))
        #
        # plt.figure('delta')
        # plt.plot(time_debug[0:-1], opti.debug.value(delta))
        #
        # plt.figure('throttle')
        # plt.plot(time_debug[0:-1], opti.debug.value(throttle))
        ...
        print('------------------------------')
        print('integrand = plt.squeeze([ax_cl]**2)')
        print(sol.value(f0_cal))
        print('integrand = plt.squeeze([ay_cl] ** 2)')
        print(sol.value(f1_cal))
        print('integrand = plt.squeeze([jy_cl] ** 2)')
        print(sol.value(f2_cal))
        print('integrand = plt.squeeze((delta_lane - [y_cl]) ** 2)')
        print(sol.value(f3_cal))
        print('integrand = plt.squeeze((desired_speed - [vx_cl]) ** 2)')
