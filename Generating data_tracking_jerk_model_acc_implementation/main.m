...
% legend('R\_axtot acc','C\_axtot acc','Location','southeast')
subplot(2,2,4)
plot(t_mpc(1:5*2:N_ame),abs(throttle_dot_mpc-throttle_ref(1:2*2:N_ref)),'b','LineWidth',1.0)
title('Error [-]','fontsize',12,'fontweight','bold')
xlabel('t [s]','fontsize',12)
xlim([0, t_max])
ylabel('throttle\_dot error  [1/s]','fontsize',12)
saveas(gcf,".\written_data\10tr_dot&delta_dot_N"+string(N)+"_TMPC "+string(T_MPC)+"_Tf"+string(Tf)+".png")
saveas(gcf,".\written_data\10tr_dot&delta_dot_N"+string(N)+"_TMPC "+string(T_MPC)+"_Tf"+string(Tf)+".fig")
