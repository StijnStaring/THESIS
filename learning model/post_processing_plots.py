def post_processing_plots(his_f_calc_rel,his_weights,his_multi_grads,his_grad_current,his_diff_theta,file_list):
    ...
    for d in plt.arange(0, len(file_list), 1):
        ...
        plt.figure("Convergence of features "+ file[15:-4], figsize=(24, 12))
        plt.tight_layout()
        plt.subplot(2, 3, 1)
        acf1 = plt.gca()
        ...
        for i in plt.arange(0, len(iterations), 1):
            ...
            acf6.plot(iterations[i], f_calc_rel_curr[i][5], 'o', label="iteration " + str(i + 1), linewidth=3.0)

        f_calc_rel_curr = []

        # history of current gradient
        plt.figure("History of current gradient " + file[15:-4], figsize=(24, 12))
        plt.tight_layout()
        plt.subplot(2, 3, 1)
        acgc1 = plt.gca()
        # plt.xlabel("Iteration [-]", fontsize=14)
        ...
        for i in plt.arange(0, len(iterations), 1):
            ...
            acgc6.plot(iterations[i], grad_curr[i][5], 'o', label="iteration " + str(i + 1), linewidth=3.0)
