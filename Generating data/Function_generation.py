...
lane distances: single (3.47 m), double (6.94 m)
:return: csv-datafiles

Assumptions:
Assuming that longitudinal velocity at the start is equal to the desired velocity of the lane change.
...
data_guess_list = import_ideal_data()
# time_guess = data_cl['time_guess']
...
# vx_start = plt.array([80/3.6])
# width_road = plt.array([3.46990715])
...
#  Set an guess
# first make an estimate how far the lane change is completed --> assuming constant vx and small yaw angle
# shape of guess of states: 1,N+1
if plt.around(width_road,0) == 3.0:
    data_cl = data_guess_list[0]
else:
    data_cl  = data_guess_list[1] # 6.94 m (double lane change)
# F = Function('F', [states, controls, dt], [states_next],['states','controls','dt'],['states_next'])

# sample = X0[0]/(X0[2]*data_cl['dt_cl'])
sample_function = Function('sample_function', [X0], [X0[0]/(X0[2]*data_cl['dt_cl'])],['X0'],['sample'])

# time_guess = Function('time_guess',[time],[ time+5 ])
# index --> sample 1

opti.set_initial(T, time_guess)


x_guess = plt.linspace(X0[0],vx_start*time_guess,N+1)
x_guess = x_guess[plt.newaxis,:]
opti.set_initial(x,x_guess)
vx_guess = vx_start * plt.ones((1,N+1))
opti.set_initial(vx,vx_guess)

_,index_y = find_nearest(data_cl['y_cl'],X0[1])
y_guess = signal.resample(data_cl['y_cl'][index_y:],N+1)
opti.set_initial(y,y_guess)

vy_guess = signal.resample(data_cl['vy_cl'][index_y:],N+1)
opti.set_initial(vy,vy_guess)

psi_guess = signal.resample(data_cl['psi_cl'][index_y:],N+1)
opti.set_initial(psi,psi_guess)

psi_dot_guess = signal.resample(data_cl['psi_dot_cl'][index_y:],N+1)
opti.set_initial(psi_dot,psi_dot_guess)

throttle_guess = signal.resample(data_cl['throttle_cl'][index_y:],N)
opti.set_initial(throttle,throttle_guess)

delta_guess = signal.resample(data_cl['delta_cl'][index_y:],N)
opti.set_initial(throttle,throttle_guess)
...
opti.subject_to(jy_tot[-1] == 0) # end of lane change --> no lateral acceleration in the next sample
...
# ---------------------------------------------------
#    Convert opti environment to a CasADi function
# ---------------------------------------------------
